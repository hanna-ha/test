            ...
            html.A(
                html.Div(
                    ...
                    style={
                        ...
                        "height": "150px",
                        "lineHeight": "150px",
                        "border": "2px dashed",  # #009539 : biospyder green
                        "borderRadius": "5px",
                        "textAlign": "center",
                        "fontSize": "20px",
                        "fontWeight": "bold",
...
# # Callback for project selection status and user info display
# @callback(
...
# def handle_project_selection( userpath , selected_project , n_click):
...
#         ])
#         return status_message, user_message , selected_project

#     else:
#         print('here? why ? I dont undrestand ' )

#         upload_folder = f"dash_app/assets/user_upload_test" # create foldr if not avilable
...
#             if not folders:
#                 return dbc.Alert("No upload data found.", color="warning"), ""
...
#             if not files:
#                 return dbc.Alert(f"No files found in folder {most_recent_folder}.", color="warning"), ""
...
#             return dbc.Alert(f"An error occurred while loading the data: {str(e)}", color="danger"), "" ,""

#         return status_message, user_message , most_recent_file
...
#     # prevent_initial_call=True
# )
# def handle_project_selection(userpath, selected_project, n_click, stored_n_click, stored_selected_project):
#     print(f"userpath, selected_project, n_click: {userpath, selected_project, n_click}")
#     print(f"stored_n_click, stored_selected_project: {stored_n_click, stored_selected_project}")
    
#     username = userpath.replace('/data/reads/', '')

#     # Use stored values if inputs are None
#     if n_click is None:
#         n_click = stored_n_click
#     if selected_project is None:
#         selected_project = stored_selected_project

#     if n_click is None:
#         if not selected_project:
#             return "", f"Hi {username}", "", stored_n_click, stored_selected_project
...
#             html.P(f"Selected data: {selected_project}", style={"fontWeight": "bold"})
#         ])
#         return status_message, user_message, selected_project, stored_n_click, selected_project
...
#             folders = [f for f in os.listdir(upload_folder) if os.path.isdir(os.path.join(upload_folder, f))]
#             if not folders:
#                 return dbc.Alert("No upload data found.", color="warning"), "", stored_n_click, stored_selected_project
...
#             if not files:
#                 return dbc.Alert(f"No files found in folder {most_recent_folder}.", color="warning"), "", stored_n_click, stored_selected_project
...
#         except Exception as e:
#             return dbc.Alert(f"An error occurred while loading the data: {str(e)}", color="danger"), "", stored_n_click, stored_selected_project

#         return status_message, user_message, os.path.basename(most_recent_file), n_click, selected_project
